	...
	methods: {
		...
		checkRegex(x, e, idx) {
			if (e.target.value.length < 1) {
				...
				console.log(this.errorcode.includes(this.formDate[idx].title))
				if (!this.errorcode.includes(this.formDate[idx].title)) {
		...
		confirmMessage() {
				...
				for (let j of Object.keys(this.submitForm)) {
					...
					if (this.submitForm[j].length < 1) {
						for (let k of this.formDate) {
							if (k.name == j) {
								k.checkresult = 'empty'
								if (!this.errorcode.includes(k.name)) {
			...
			if (this.wrongcode.length > 0 || this.errorcode.length > 0) {
				if (this.wrongcode.length > 0) {
